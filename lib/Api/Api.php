...
use Milex\QueryBuilder\QueryBuilder;
...
class Api implements LoggerAwareInterface
{
    ...
    /**
     * Array of deprecated endpoints to use if a response fails as a 404 with a previous version of Milex.
     *
     * @var array
     */
    ...
    public function makeRequest($endpoint, array $parameters = [], $method = 'GET')
    {
        ...
        if (!$bcEndpoint || !$this->bcTesting || ($bcEndpoint && $this->bcTesting && $this->bcAttempt)) {
            ...
            if (false === strpos($url, 'http')) {
                ...
                'code'    => 500,
                'message' => sprintf(
                    'URL is incomplete.  Please use %s, set the base URL as the third argument to $MilexApi->newApi(), or make $endpoint a complete URL.',
                ...
                try {
                    ...
                    $this->getLogger()->error('Failed connecting to Milex API: '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
            ...
            if (!empty($error)) {
                ...
                $this->getLogger()->error('Milex API returned errors: '.var_export($response['errors'], true));
            }
    ...
    /**
     * Returns Milex version from the HTTP response headers
     * (the header exists since Milex 2.4.0).
     *
     * @return string|null if not known
     */
    public function getMilexVersion()
    {
        $headers = $this->auth->getResponseHeaders();

        if (isset($headers['Milex-Version'])) {
            return $headers['Milex-Version'];
        }
