...
setup_milex() {
    ...
    printf "Cloning the \"features\" branch from milex/milex...\n"
    git clone -b features --single-branch --depth 1 https://github.com/milex/milex.git milex
    cd milex

    composer install --prefer-dist --no-progress
    cp ../.ddev/milex-local.php.dist ./app/config/local.php

    printf "Installing Milex...\n"
    php bin/console milex:install --force http://localhost/milex \
    ...
    printf "If you want to open the Milex instance, go to https://api-library.ddev.site/milex in your browser.\n"
    tput sgr0
}
...
then
    ...
    tput setab 3
    tput setaf 0
    printf "Do you want us to set up a Milex instance for you to test against?\n"
    printf "If you answer \"no\", you will have to set up a Milex instance yourself."
    tput sgr0
    printf "\nAnswer [yes/no]: "
    read MILEX_PREF
    ...
    if [[ $MILEX_PREF == "yes" ]]
    then
        printf "Okay, setting up a Milex instance...\n"
        echo "ddev-managed" > ./.ddev/milex-preference
        setup_milex
    else
        printf "Okay, you'll have to set up a Milex instance yourself. "
        printf "Copy /tests/local.config.php.dist to /tests/local.config.php and add your Milex instance settings.\n"
        echo "unmanaged" > ./.ddev/milex-preference
    fi
else
    # Ensure our milex/milex clone is up-to-date
    echo "Updating the cloned Milex instance..."
    cd milex
    git pull
    # Need to downgrade to Composer v1 until Milex 4 is out
